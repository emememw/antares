...
import com.emveyh.antares.ui.menu.Menu;
	...
	private void renderWorldMap(SpriteBatch batch) {
		...
		for (int x = 0; x < MapManager.getInstance().getWorld().getTiles().length; x++) {
			for (int y = 0; y < MapManager.getInstance().getWorld().getTiles()[x].length; y++) {
				if (MapManager.getInstance().getWorld().getTiles()[x][y] != Tile.WATER && MapManager.getInstance().getWorld().getTiles()[x][y] != Tile.RIVER) {
		...
		for (int x = 0; x < MapManager.getInstance().getWorld().getTiles().length; x++) {
			for (int y = 0; y < MapManager.getInstance().getWorld().getTiles()[x].length; y++) {
				if ((int) (EntityManager.getInstance().getPlayer().getX() / GlobalConfig.FIXED_TILESIZE) == x
					...
					batch.draw(TextureManager.getInstance().getSprites()[3][0], x * tileSize + xOffset + CameraManager.getInstance().getCamera().position.x, y
							* tileSize + yOffset + CameraManager.getInstance().getCamera().position.y, 8, 8);
				}
